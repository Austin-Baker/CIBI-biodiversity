...
Order_Family_df <- BIN_Order %>%
  select(Order, Order_Family) %>%
  unique()

# loop for every family to combine BINs
maps_done <- str_remove(list.files("Order_maps/"), ".tif")
Order_loop <- unique(Order_Family_df$Order)
Order_loop <- Order_loop[!Order_loop%in%maps_done]

for(f in Order_loop){
  BIN_Order_Family <- Order_Family_df %>%
    filter(Order==f) 
  
  f_paths <- paste0("Family_maps/", BIN_Order_Family$Order_Family, ".tif")
  ...
  writeRaster(combined_raster, paste0("Order_maps/", f, ".tif"), overwrite = TRUE)
